...
export const updateMetricsFromDealEvent = async (dealData: any): Promise<void> => {
  const { type, deal_value, forecast_value, stage, expected_close_date, org_id, previous_stage } = dealData;
  ...
  // Check if this is a new deal or an update by making an API call to the Deal microservice
  let existingDeal;
  /*try {
    const response = await axios.get(`http://localhost:5002/api/deals/${dealData._id}`);
    existingDeal = response.data; // Assuming the response contains the deal data
    console.log("metrics data : ", existingDeal);
  } catch (error: unknown) {
    if (axios.isAxiosError(error)) { // Use axios's type guard
      if (error.response && error.response.status === 404) {
        // Deal not found, meaning this is a new deal
        existingDeal = null;
      } else {
        // Handle other Axios errors (e.g., network errors, unexpected response)
        console.error('Error fetching deal from microservice:', error.message);
        throw new Error('Unable to fetch deal information. Please try again later.');
      }
    } else {
  ...
  //console.log(type);
  // Handle metrics update based on deal type
  if (type === 'create') {
    // This is a new deal
